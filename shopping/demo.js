...
let passInput = document.querySelectorAll(".user .password");
let passInputs = document.querySelectorAll(".new-user .password");
...
// 確認密碼
function validatePassword(passwordInput, errorSelector) {
...
function handleLogin() {
  ...
  const isPassValid = validatePassword(passInput, ".user .password-error");
...
function handleRegister() {
  ...
  // 執行相應的驗證功能
  const isEmailValid = checkEmail(emailInput);
  const isPassValid = validatePassword(passInput, ".new-user .password-error");
...
// 添加 cpassword 欄位的眼睛圖示功能
const cEyeIcons = document.querySelectorAll(".new-user .fa-eye-slash");

cEyeIcons.forEach((eyeIcon) => {
  eyeIcon.addEventListener("click", () => {
    const cpasswordInput = eyeIcon.parentElement.querySelector(".cpassword");

    // 檢查 cpasswordInput 是否存在
    if (cpasswordInput) {
